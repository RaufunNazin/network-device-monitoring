...
def main(debug_mode):
    ...
    mib_compiler = MibCompiler(
        SmiStarParser(), PySnmpCodeGen(), PyFileWriter(output_mib_dir)
    )
    mib_compiler.add_sources(FileReader(source_mib_dir))

    mib_files = [
        f
        for f in os.listdir(source_mib_dir)
        if os.path.isfile(os.path.join(source_mib_dir, f))
    ]
    ...
    for dep in dependencies:
        if dep in mib_names:
            try:
                ...
                print(f"{dep}: {result.get(dep, 'unknown result')}")
            except Exception as e:
                print(f"Error compiling dependency {dep}: {e}")

    print("\nCompiling remaining MIBs...")
    ...
    print(
        f"\nAttempted all mib compilation into py modules. Check {output_mib_dir} for results."
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compile MIB files with optional debug logging"
    )
    parser.add_argument(
        "-d",
        "--debug",
